...
usa_all <- read_delim('raw/usa_all.txt', delim = '\t', trim_ws = TRUE) %>%
  ...
  drop_na() %>%
  rename(Year = `Year Code`, Sex = `Gender Code`, Age = `Single-Year Ages Code`,
         Death = Deaths) %>%
...
cleaned_usa <- raw_usa %>%
  rbind(usa_residual) %>%
  rbind(usa_all) %>%
  mutate(Country = 2450, Name = 'USA', Age_width = ifelse(Age==100, Inf, 1),
         Cause = factor(Cause, level = c(cause_label, 'All'))) %>%
  arrange(Country, Name, Year, Sex, Age, Age_width, Cause, Death) %>%
  select(Country, Name, Year, Sex, Age, Age_width, Cause, Death)

## by 5-year age group: the open-ended age group for the USA in WHOMD is 95+
y5_usa <- cleaned_usa %>%
  mutate(
    Age = case_when(
      Age==0 ~ 0,
      Age>=1 & Age<=4 ~ 1,
      Age>=5 & Age<=94 ~ as.integer(Age/5)*5,
      Age>=95 ~ 95
    ),
    Age_width = case_when(
      Age==0 ~ 1,
