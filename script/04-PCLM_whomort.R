...
# USA
usa_plot <- cod_sa %>%
  filter(Name=='USA', Year==2022) %>%
  mutate(Type = 'PCLM-estimated') %>%
  rbind(extra$sa %>% mutate(Type = 'Original')) %>%
  ggplot() +
  facet_wrap(~ Cause + Sex, scales = 'free_y') +
  geom_line(aes(x = Age, y = Death, color = Type)) +
  scale_color_manual(values = c(`PCLM-estimated` = 'blue', `Original` = 'red')) +
  labs(title = 'Compare Original and PCLM-estimated USA 2022 Death Counts', 
       x = 'Age', y = 'Mx', color = 'Type') +
  theme_custom()

# use the original single-age data rather than the PCLM-estimated for USA-2021
cod_sa <- cod_sa %>%
  filter(!(Name=='USA' & Year==2022)) %>%
  rbind(extra$sa)
...
pdf('output/pclm_diagnostics_2022_usa.pdf')
pdf.options(width = 16, height = 12)
print(usa_plot)
dev.off()
